...
const Singelproduct2 = () => {
  ...
  const getSingleProduct = async (id) => {
    try {
      const res = await fetch(
        `https://my-kart-server.vercel.app/api/v1/getsingleproduct/${id}`
      );
  ...
  if (error) {
    return (
      <p className=" my-16  text-2xl font-semibold text-center ">{error}</p>
    );
  }
    ...
    <div className="my-6 md:my-12 sm:p-8 p-5 flex flex-col md:flex-row md:space-x-8 md:space-y-0 space-y-5 justify-center">
      <div className="flex flex-row items-center space-x-6 justify-center">
          ...
          {singleSaman.image.map((url, index) => {
            return (
              <img
                src={url}
                key={index}
                className="sm:h-[90px] h-[60px] sm:w-[140px] w-[100px]  hover:border-2 cursor-pointer border-slate-500"
                alt="product-img"
                onMouseOver={() => {
                  setimgurl(url);
                }}
        ...
        <div id="full-img">
          <img
            src={imgurl}
            className="sm:h-[190px] h-[150px] w-[220px]  sm:w-[280px] "
            alt="product-img"
          />
        </div>
      </div>
      ...
      <div className="" id="product-detail">
        ...
        <span className="px-2 py-1 bg-nav rounded-xl mt-3 inline-block">
          {singleSaman.stars} <FaStar className="inline-block mb-1" />
        </span>
        <span className="mx-1">{singleSaman.reviews} reviews</span>
        <p className="mt-3 font-semibold">
          <FormatPrice price={singleSaman.price} />
        </p>
        <p className=" md:max-w-[400px] mt-5">{singleSaman.description}</p>
        <hr className="mt-5"></hr>
        <p className="mt-2">
          <span className="font-semibold ">Avaliable:</span> {singleSaman.stock}{" "}
          in stock
        </p>
        <p className="">
          <span className="font-semibold ">Company:</span> {singleSaman.company}
        </p>
        <p className="mt-2">
          <span className="font-semibold ">Color:</span>{" "}
          <Showcolor singleSaman={singleSaman} colorsetting={colorsetting} />
        </p>
        <SetAmount amount={amount} Inc={Inc} Dec={Dec} />
        <button
          className="bg-nav px-[6px] py-1 rounded-lg ml-4 disabled:opacity-60 hover:text-black active:scale-105"
          disabled={!user}
          onClick={() => {
