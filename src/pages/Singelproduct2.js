...
const Singelproduct2 = () => {
    ...
    const getSingleProduct = async (id) => {
        try {
          ...
          setsingleSaman(singleProduct)
          setimgurl(singleProduct.image[0].url)
...
function Dec(){
 ...
 if(isSingleLoading){
    return <img src={spinner} className='h-[110px] w-[110px] mx-auto my-10'  alt='spinner' />
}
if(error){
  ...
  return (
    ...
    <div className="flex flex-row items-center space-x-6 justify-center">
         <div className="flex flex-col space-y-3">
             {singleSaman.image.map((elm,index)=>{
              return(
                <img src={elm.url} key={index} className='sm:h-[90px] h-[60px] sm:w-[140px] w-[100px]  hover:border-2 cursor-pointer border-slate-500' alt='product-img'onMouseOver={()=>{
                  setimgurl(elm.url)
                 }}  />
                  )})}
          </div>
  ...
  <div className="" id='product-detail'>
       <h1 className="font-semibold md:text-3xl text-2xl">{singleSaman.name}</h1>
         ...
         <p className=""><span className="font-semibold ">Id:</span> {singleSaman.id}</p>
        ...
        <button  className="bg-nav px-[6px] py-1 rounded-lg ml-4" onClick={()=>{
