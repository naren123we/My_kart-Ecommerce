...
const Product = ({elm,lefthandel,righthandel}) => {
  ...
  return (
    <div className={`flex flex-col w-[100vw] sm:w-[30%] md:w-[25%] m-10px h-[auto] hover:shadow-lg bg-white px-2 pt-2  cursor-pointer relative  rounded-xl`}  id='product-card'>
           <AiOutlineLeft   className='absolute text-4xl sm:hidden  text-white font-bold left-4  bottom-[32vw]' onClick={lefthandel}/>
     
      <AiOutlineRight  className='absolute text-4xl sm:hidden text-white  font-bold   right-4     bottom-[32vw]' onClick={righthandel} />
