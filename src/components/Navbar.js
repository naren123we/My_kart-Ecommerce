...
import { AiOutlineMenu,AiFillHome,AiFillContacts,AiFillAppstore } from 'react-icons/ai';
...
const Navbar = () => {
  ...
  return (
    <nav className='bg-nav   shadow-lg relative ' id="nav-top" >
     ...
     <ul className={`md:flex  hidden lg:space-x-14 space-x-10 items-center justfy-between `}>
      ...
      <div className={`${mobile_menu?'-translate-x-[400px]':' translate-x-0 '}
       top-0 z-10  animation-ul  w-[50vw] shadow-2xl h-[100vh] md:hidden
        bg-white text-black absolute `}>
            <p className='h-[9vh] w-[100%] bg-nav '></p>
     <ul className={`flex flex-col mt-5 mb-3 space-y-2`}>
       
    


       <li  className='flex mt-8 font-semibold px-3 sm:px-5 pt-2  hover:text-slate-400 cursor-pointer text-xl'><AiFillHome className=' mx-[4px] mt-[3px]'/><NavLink to='/'>Home</NavLink></li>
       <hr></hr>
